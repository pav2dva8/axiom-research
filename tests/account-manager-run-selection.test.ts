...
test("run selection is independent from accounts selection and only loads valid tokens", async () => {
  const originalCwd = process.cwd();
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "axiom-run-selection-"));
  ...
  try {
    ...
    manager.setRunSelection([publicKeys[1], publicKeys[2]]);

    assert.deepEqual(
      manager.listAccounts().filter((account) => account.selected).map((account) => account.publicKey),
      [publicKeys[0]],
    );
    assert.deepEqual(
      manager.listRunAccounts().filter((account) => account.selected).map((account) => account.publicKey),
      [publicKeys[1]],
    );
    assert.deepEqual(
