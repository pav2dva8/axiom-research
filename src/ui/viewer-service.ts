...
export class ViewerService extends EventEmitter {
  ...
  async fetchTokenInfo(pairAddress: string): Promise<TokenInfo | null> {
    ...
    let data: any = null;
    try {
      data = await tryHost("api9.axiom.trade");
    } catch {}
    if (!data) {
      try {
        data = await tryHost("api2.axiom.trade");
      } catch {}
    }
    if (!data) return null;
    return {
  ...
  private async connectAccount(
    ...
    try {
      ...
      if (
        ...
        console.log(
          `[Viewer] ${account.publicKey.slice(0, 8)} bootstrap SKIPPED (UI toggle)`,
        );
      }
    ...
    const worker = async (slotIndex: number) => {
      while (true) {
        ...
        if (this.connectCancelled) {
          const id = this.connectedViewers.get(account.publicKey);
  ...
  async disconnectSlowly(delayMs = 2000): Promise<number> {
    if (!this.browserSession) return 0;
    this.connectCancelled = true;
    ...
    while (this.connectedViewers.size > 0 && !this.slowStopCancelled) {
      ...
      if (!entry) break;
      const [publicKey, viewerId] = entry;
      try {
        await this.browserSession.disconnectViewer(viewerId);
      } catch {}
